...
description: "Use when implementing an approved Habitify Python client slice, translating a plan into code while preserving repo conventions such as httpx transport, resource-style APIs like habitipy.habits.list(...), and narrow validation."
...
3. Run the narrowest useful validation first, then ensure test-related validation covers every supported Python version in the declared support matrix when applicable.
4. Report what changed, how it was validated, and whether a checkpoint commit was possible.
