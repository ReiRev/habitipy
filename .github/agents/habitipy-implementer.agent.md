...
- DO NOT skip `poetry run isort`, `poetry run black`, `poetry run ruff check`, and `poetry run mypy habitipie` after Python edits.
...
3. Run `poetry run isort`, `poetry run black`, `poetry run ruff check`, and `poetry run mypy habitipie` on the touched Python files.
